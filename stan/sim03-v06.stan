...
model {
  ...
  for (i in 1:N) {
    ...
    // first point uses marginal sd
    target += normal_lpdf(y[i, 1] | mu[1], sigma);
    
    // rest are conditional and have SD = sigma * sqrt(1 - rho^2)
    for (j in 2:J) {
      target += normal_lpdf(
        y[i, j] | mu[j] + rho * (y[i, j-1] - mu[j-1]), innov_sd);
    }
  }
