...
While the data are simulated using a linear risk model, the simulation model of the adaptive design is specified as a multivariable logistic regression model from which we subsequently transform the parameters back to the risk scale via the inverse link and a g-computation step.
The model form is:
...
In practice, the log-odds of treatment failure may be differential within the strata, but this has been ignored for now and no interactions are included.
...
1. Ordinal representation of the primary outcome
2. Integrating re-randomisation into the design
...
The fixed design result corresponds to a single analysis under the same data generating process, assessing both NI and inferiority at the same decision thresholds and evidential levels.
...
for(i in 1:length(l)){
  ...
  # extract the decision matrix - sim, analysis, quantity, domain level decision
  d_dec_1 <- copy(l0[[i]]$d_pr_dec[par %in% c("rd_2_1", "rd_3_1")])
  # config for scenario
...
g_tbl <- d_tbl_1_cur |> 
  ...
  cols_merge(
    columns = c("ref_ni_600", "ref_inf_600"
                ),
    pattern = "<<{1}>><< ({2})>>"
  )  |>
  cols_width(
  ...
  cols_label(
    rd = "RD relative to SoC",
    ni_300 = html("300"),
    ni_400 = html("400"),
    ni_500 = html("500"),
    ni_600 = html("600"),
    ref_ni_600 = html("Fixed design")
  ) |>
  tab_options(
    table.font.size = pct(55),
...
for(i in 1:length(l)){
  ...
  d_pars <- d_pars[par %in% c("rd_2_1", "rd_3_1"), .(sim, par, mu)]
...
d_tbl_2_cur <- d_post_1_ref[,
                 .(mu = mean(mu),
                   q_025 = quantile(mu, prob = 0.025),
                   q_975 = quantile(mu, prob = 0.975)), 
                 keyby = .(scenario, desc, par, N)]
# setorderv(d_fig, cols = "scenario", order = -1L)
d_tbl_2_cur[, desc := factor(desc, levels = unique(d_post_1$desc))]

d_tbl_2_cur[par == "rd_2_1", par := "Deferred"]
d_tbl_2_cur[par == "rd_3_1", par := "Discontinued"]

d_tbl_2_cur <- dcast(d_tbl_2_cur, scenario + desc + par ~ N, value.var = list("mu", "q_025", "q_975"))

setnames(d_tbl_2_cur, c("mu_600", "q_025_600", "q_975_600"), c("mu_ref", "q_025_ref", "q_975_ref"))

d_tbl_1_cur <- merge(d_tbl_1_cur, d_tbl_2_cur, by = c("scenario", "desc", "par"))
