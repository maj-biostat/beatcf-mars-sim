...
for(i in 1:length(l)){
  ...
  d_pr_dec <- copy(l[[i]]$d_pr_dec[trt %in% c("defer", "discont") & rule == "ni"])
...
g_tbl <- d_tbl_1_cur |> 
  ...
  fmt_number(
    columns = c("mu_discont", "mu_defer", "lo_discont", "lo_defer", "hi_discont", "hi_defer"), 
  ...
  cols_merge(
    columns = c("mu_discont", "lo_discont", "hi_discont"
                ),
    pattern = "<<{1}>><< ({2}, {3})>>"
  )   |> 
  # cols_width(
...
# we are only interested in defer vs soc and discont vs soc
d_b_trt <- data.table()
...
g_tbl <- d_tbl_1_cur |> 
  ...
  cols_merge(
    columns = c("N_mu_discont", "N_sd_discont"
                ),
    pattern = "<<{1}>><< ({2})>>"
  ) |>
  cols_merge(
