...
Currently, a baseline reading (where treatment assignment is assumed to have no differential effect) and `r l_spec_1$n_sprty_obs` equally spaced follow up visits are assumed.

At the start of the study, participants are randomised with equal allocation to one of the three strategies at enrolment and this allocation is assumed to remain constant over the 12 month period.
...
Of the 121 enrolments, 94 had at least one PEx and among the 33 assigned to tailored therapy, only 10 received oral antibiotics which was higher than the expected (hypothesisded) <10% that would avoid antibiotics.
...
For example, the observed ppFEV1 values and the mean along with the residual autocorrelation function for participants at the approxiamte `r paste0(100*q_p, "%", collapse = ", ")` percentiles for baseline age are shown in @fig-ppfev1-age0-ex.
...
As another example, observed ppFEV1 for a random selection of `r l_spec_tmp$N` participants with no differential treatment group effects and followed over 12 months are shown in @fig-ppfev1-age-prog.
...
d_dec_pars <- data.table(
  desc = c("NI", "Futility"),
  ref_value = c(l_spec_1$delta$ni, l_spec_1$delta$fut),
  threshold = c(l_spec_1$thresh$ni, l_spec_1$thresh$fut)
)
...
Conversely, if the ppFEV1 on the intervention arm falls below the futility reference vaule with high probability, then we conclude futility.
