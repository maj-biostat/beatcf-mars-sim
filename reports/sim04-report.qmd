...
|:----|:------------|:----|:------------|
| 0.1 | 2025-11-10 | First version | N/A |
...
Currently, a baseline reading (prior to treatment assignment) and `r l_spec_1$n_sprty_obs` follow up visits are assumed (equally spaced over the 12 months).
...
We simulate ppFEV1 at baseline and over the 12 months of follow up, assuming that ppFEV1 progression can be adequately modelled based on baseline ppFEV1 value (in a well state), age at entry, a time trend and treatment effect.
While not currently considered in the data generation process, it may be worth incorporating the distribution of exacerbations.
...
The $B_k$ provide a piecewise linear basis that operate on $t_{i,j}$ assuming a linear progression of ppFEV1 within the nominated segements (quarterly).
...
The outcome (observed ppFEV1) is regressed on the (observed) baseline value for ppFEV1, log(age0), time and treatment assignment, using piecewise regression and generalised least squares:
