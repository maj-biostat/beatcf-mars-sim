...
2. Follow up at multiple timepoints rather than just pre/post
3. A re-randomisation design could be considered; patients would be randomised on each exacerbation
4. Some level of stratification may be considered
5. Alternative followup timepoint
6. ...
...
The initial standard of care remains the reference group throughout and the trial stops once all questions are resolved or the maximum sample size of `r sum(l[[1]]$cfg$N_pt)` reached.
...
The data are simulated using a saturated linear model with terms for intercept and slope (with the intercept giving the population average mean ppFEV1 and the sum of the two yielding the mean ppFEV1 at follow up) along with participant level deviations on both the intercept and slope.
...
The outcome is modelled using a pre/post linear model that includes a term for treatment assignment (ANCOVA).
...
Each scenario adopts a maximum sample size of 600 with interim analyses run when `r paste0(cumsum(l[[1]]$cfg$N_pt)[-length(l[[1]]$cfg$N_pt)], collapse = ", ")` enrolments have occurred.
...
The final analysis is assumed to occur at the maximum followup time.
...
# 
N <- cumsum(l[[1]]$cfg$N_pt)[-length(l[[1]]$cfg$N_pt)]
...
ggplot(d_fig_1, aes(x = value, y = id, 
  ...
  scale_y_continuous("Cumulative enrolments",
                     breaks = seq(0, sum(l[[1]]$cfg$N_pt), by = 100)) +
...
for(i in 1:length(l)){
  ...
  d_all <- copy(l[[i]]$d_all[sim == 1, .(sim, ia, id, trt, fu, y, y_mis)])
  
  d_all[, fu := factor(fu, levels = c(0, 1), labels = c("baseline", "followup"))]
...
d_fig_2 <- d_ppfev1[
  y_mis == "observed", .(y_mu = mean(y)), keyby = .(scenario, desc, trt, fu)]

ggplot(data = d_fig_1, aes(x = fu, y = y, group = id)) +
  geom_line(alpha = 0.5, lwd = 0.2) +
  geom_line(
    data = d_fig_2,
    aes(x = fu, y = y_mu, group = trt),
...
for(i in 1:length(l)){
  ...
  d_enrolment <- data.table(
    ic = seq_along(l_cfg$N_pt), N_enrol = cumsum(l_cfg$N_pt))

  # observed data
  d_all <- copy(l[[i]]$d_all)
  d_all <- base::merge(d_all, d_enrolment , by = "ic")
  d_all <- d_all[fu == 1 & y_mis == 0, .(mu_obs = mean(y)), keyby = .(sim, trt)]
