...
For the data generation, we are modeling the outcome at each timepoint and assuming that the treatment effect manifests at 12 months and so the linear combination of terms used to simulate the outcome variable at baseline and followup is:
...
Accrual is assumed to follow a non-homogenous poisson process ramping up over `r l[[1]]$cfg$ramp_up_days` days with around `r l[[1]]$cfg$pt_per_day` enrolments per day.
