...
get_sim14_pt <- function(
  ...
  while (t < l_spec$followup) {
    ...
    if (state == "H") {
      ...
      # for the period of time until the first exacerbation, we do not include 
      # the carry over effects of treatment because no treatment has been 
      # delivered to the participant yet in the rct
      if(length(states_list) == 0){
        # draw gap time H->E
        trt <- "none"
        lambda <- u_he * exp(l_spec$mu_he + 
                               l_spec$b_ppfev_he * ppfev_baseline)
      } else {
        # retrieve the last trt allocated 
        trt <- states_list[[length(states_list)]]$trt
        stopifnot("Retrieved trt must be one of the active trt "= trt %in% l_spec$trt_lab[l_spec$trt_active])
        lambda <- u_he * exp(l_spec$mu_he + 
                               l_spec$b_ppfev_he * ppfev_baseline + 
                               l_spec$b_trt_he[trt])
      }
      ...
      if (t_next > l_spec$followup) {
        ...
        # will be censored since we do not see the start of the next exacerbation
        states_list[[length(states_list)+1]] <-
          data.table(ix = i_count, state="H", start=t, stop=l_spec$followup, trt=trt, complete = 0)
        
        break
      }
      
      states_list[[length(states_list)+1]] <-
        data.table(ix = i_count, state="H", start=t, stop=t_next, trt=trt, complete = 1)
      ...
      lambda <- u_eh * exp(l_spec$mu_eh + 
                      l_spec$b_ppfev_eh * ppfev_baseline + 
                      l_spec$b_trt_eh[trt])
      ...
      if (t_next > l_spec$followup) {
        states_list[[length(states_list)+1]] <-
          data.table(ix = i_count, state="E", start=t, stop=l_spec$followup, trt=trt, complete = 0)
        break
      }
      
      states_list[[length(states_list)+1]] <-
        data.table(ix = i_count, state="E", start=t, stop=t_next, trt=trt, complete = 1)
...
#' to create cohort
#' 
get_sim14_cohort <- function(l_spec){
  ...
  for(i in seq_along(id_cohort)){
    ...
    pt_list[[i]] <- cbind(id = id_cohort[i],  t0 = l_spec$t0[i], get_sim14_pt(l_spec))
...
#' 
get_sim14_stan_data <- function(dd){
  
  # Those that have exacerbations
  d_tmp <- copy(dd[state == "E"])
  d_tmp[, ppfev_std := ppfev_baseline]
  d_tmp[, defer := as.numeric(trt == "defer")]
  d_tmp[, discont := as.numeric(trt == "discont")]
  
  # A sojourn is censored if the patient was still in this state 
  # at followup end i.e. we never see them recover and as such their stop time 
  # will equal the followup duration exactly and the complete indicator will 
  # be set to zero - we never see the transition back to the health state:
  d_tmp[, censored := copy(1-complete)]
  
  # rejig the id indexs so that they are contiguous
  d_tmp[, id_idx := as.integer(factor(id))]
  # number of unique pt with exac in this analysis:
  ...
  # d_cens contains subjects whose final state was a E sojourn where we did not
  # see the transition back to health.
  # d_obs contains subjects who completed at least one E->H transition in full.
  ...
  # d_obs or d_cens, as they have no contribution to the E->H likelihood.
  ...
  ld_eh <- list(
    ...
    X_cens  = X_cens,
    # index for each occurrence of state within each pt
    ix_state_obs = d_obs$ix,
    ix_state_cens = d_cens$ix,
    # dimensions
    N_id    = N_id,
    P       = ncol(X_obs),
    compute_rmst = 1,
  ...
  # we include trt from last exacerbation to account for carry over effects
  X_obs <- model.matrix(~-1 + ppfev_std + defer + discont, data = d_obs)
  X_cens <- model.matrix(~-1 + ppfev_std + defer + discont, data = d_cens)
...
example_stan <- function(){
  ...
  m3 <- cmdstanr::cmdstan_model("stan/sim14-v01.stan")
  ...
  d_cohort <- get_sim14_cohort(l_spec)
  ...
  foutname <- paste0(
    format(Sys.time(), format = "%Y%m%d%H%M%S"), "sim14")
