...
get_sim15_pt <- function(
  ...
  while (t < l_spec$followup) {
    ...
    repeat{
      ...
      if (state == "H") {
        bin_ix <- findInterval(days_in_state, l_spec$he_bins)
        if(exac_count == 0){
          trt <- "none"
          lp <- l_spec$mu_he_0[bin_ix] + he_u + 
        ...
        bin_ix <- findInterval(days_in_state, l_spec$eh_bins)
        exac_count <- exac_count + 1
        lp <- l_spec$mu_eh_0[bin_ix] + eh_u + 
          l_spec$b_ppfev_eh * ppfev_baseline + 
          l_spec$b_trt_eh[trt]
        y <- rbinom(1, size = 1, prob = plogis(lp))
      }
  ...
  d_pt[, age := age + t/l_spec$followup]
  d_pt
...
example_cohort <- function(){
  ...
  d_cohort[, mean(dur), keyby = state]
  summary(d_cohort[, .N, keyby = id][, N])
...
#' 
get_sim15_stan_data <- function(dd){
  
  dd[, state := factor(state, levels = c("H", "E"))]
  
  dd[state == "H", bin := findInterval(d, l_spec$he_bins)]
  dd[state == "E", bin := findInterval(d, l_spec$eh_bins)]
  
  dd[trt == "defer", defer := 1]
  dd[trt != "defer", defer := 0]
  
  dd[trt == "discont", discont := 1]
  dd[trt != "discont", discont := 0]
  
  dd[, run_id := rleid(state), by = id]
  
  # pick out the start and end day to create an indicate for the first period
  # of healthyness.
  d_runs <- dd[, .(
    start_day = min(t),
    end_day   = max(t),
  ...
  dd <- merge(dd, d_runs, 
              by = c("id", "state", "run_id"), all.x = T)
  dd[state == "H" & start_day == 1, i_entry := 1]
  dd[is.na(i_entry), i_entry := 0]
  
  X <- model.matrix(~-1 + ppfev_baseline + defer + discont, data = dd)


  ld <- list(
    N   = dd[, .N],
    N_id = dd[, .N, keyby = id][, .N],
    id   = dd$id,
    y = dd$y,
    # 1 is healthy, 2 is exacerbation
    state = dd[, as.integer(state)],
    bin = dd$bin,
    N_he_bin = length(l_spec$mu_he_0),
    N_eh_bin = length(l_spec$mu_eh_0),
...
example_stan <- function(){
  ...
  m3 <- cmdstanr::cmdstan_model("stan/sim15-v01.stan")
  ...
  d_cohort <- get_sim15_cohort(l_spec)
  ...
  foutname <- paste0(
    format(Sys.time(), format = "%Y%m%d%H%M%S"), "sim15")
